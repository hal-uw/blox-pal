...
def setup_logging():
    log_file = f'/scratch1/08503/rnjain/blox-pal/logs/job-runs/fifo_pal_debug.log'
...
def main(args):
    ...
    while True:
        ...
        print("State of cluster {}".format(filtered_columns))
        logging.info(f"State of cluster {filtered_columns}")

        # Update metrics, prune completed jobs
        print("Calling update metrics")
        ...
        logging.info(f"{job_state.active_jobs}")
        ...
        # update metrics 
        # For jobs that have completed required num_iters, marke their lease status as false so that they exit()
        # Update cluster_state and job_state to reflect these terminated jobs
        # logging.info("Calling update metrics")
        # blox_instance.update_metrics(cluster_state, job_state)        
