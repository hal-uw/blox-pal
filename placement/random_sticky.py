...
class RandomSPlacement(object):
        ...
        **kwargs,
        ...
        for idx, job_id in enumerate(job_order):
            ...
            if job["is_running"] == False:
                ...
                placement, found = self._random_placement(job, free_gpus)
                ...
                if not found:
                    ...
                    for rev_idx in range(1, len(active_jobs) - idx):
                        ...
                        if potential_job_to_terminate["is_running"] == True:
                            ...
                            free_gpus = find_free_GPUs(gpu_df)
                            placement, found = self._random_placement(
                                job, free_gpus
                            )
    ...
    def _consolidated_placement(
        ...
        numGPUs_per_node = 4 # get this from the top
        gpus_for_job = list()
