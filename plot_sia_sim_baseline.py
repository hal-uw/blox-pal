...
for scheduler in config.keys():
    for placement_policy in placement:
        for load in config[scheduler]:
            jct_distribution = {}
            stat_fname = f"{exp_prefix}_{job_ids_to_track[0]}_{job_ids_to_track[1]}_{scheduler}_AcceptAll_{placement_policy}_workload-{load}_job_stats.json"
            print(stat_fname)
...
for i, placement in enumerate(placement_policies):
    ...
    for load in config["Fifo"]:
        data_load = data[data['workload'] == load]
        ...
        plot_data['workload'].append(load)
        plot_data['placement'].append(placement_labels[placement])
    placement_jct[placement] = avg_jcts

df_jct = pd.DataFrame(plot_data)

baseline = get_packed_sticky_avg_jct(df_jct)

for index, row in df_jct.iterrows():
    workload = row['workload']
    denominator = baseline[workload]
    norm_perfval = row['avg_jct'] / denominator
    # Add norm_perfval to a new column in the same row
    df_jct.at[index, 'norm_perf'] = norm_perfval    
...
geomean_df = df_jct.groupby('placement')['norm_perf'].apply(gmean).reset_index()
geomean_df['workload'] = 'geomean'

# Concatenate the geomean_df with the original DataFrame
result_df = pd.concat([df_jct, geomean_df], ignore_index=True)

# Sort the DataFrame for better presentation
result_df.sort_values(by=['placement', 'workload'], inplace=True)
...
#plt.tight_layout()
plt.xlabel('Sia-Philly Workload')
plt.ylabel('Avg JCT \n Packed-Sticky \nnormalized to 1')
plt.xticks(fontsize=20)
plt.yticks(fontsize=20)
plt.savefig(f"sia-philly-avg-jct-perworkload.pdf",
