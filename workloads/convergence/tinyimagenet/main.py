...
def main():
    ...
    train_dataset = ImageFolder(
        root='/users/Master/Megatron-Resource/workloads/convergence/tinyimagenet/tiny-imagenet-200/train',
        transform=train_transform
    )
    val_dataset = ImageFolder(
        root='/users/Master/Megatron-Resource/workloads/convergence/tinyimagenet/tiny-imagenet-200/val',
    ...
    if local_rank == 0:
        with open(f"../info/tinyimagenet_{model_name}_{dist.get_world_size() * batch_size}.txt", "w"):
            pass
    ...
    for epoch in range(num_epochs):
        ...
        if local_rank == 0:
            ...
            with open(f"../info/tinyimagenet_{model_name}_{dist.get_world_size() * batch_size}.txt", "a") as file:
                file.write(f"Epoch: {epoch}, "
                           f"Total Images: {(epoch + 1) * len(train_dataset)}, "
                           f"Accuracy: {100 * correct / total}% \n")

            print(f'Accuracy on the TinyImageNet validation images: {100 * correct / total}%')
