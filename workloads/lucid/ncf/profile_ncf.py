...
parser.add_argument("--adaptdl", action="store_true", help="enable adaptdl")
parser.add_argument('--total_time', type=int, default=30, help='Total time to run the code')
args = parser.parse_args()


def benchmark_ncf(model_name, batch_size, mixed_precision, gpu_id, bench_list):
    t_start = time.time()
    if len(gpu_id) == 1:
...
if __name__ == '__main__':
    ...
    gpu_id = [1]
    bench_list = []
    benchmark_ncf(model_name, batch_size, mixed_precision, gpu_id, bench_list)
    print(bench_list)
