...
import workloads.lucid.lstm.data as data
import workloads.lucid.lstm.models as models
...
parser.add_argument(
    ...
    help="GPU id to use. Only work when use single gpu."
)
parser.add_argument(
    "--num-warmup-batches", type=int, default=1,
    help='number of warm-up batches that don"t count towards benchmark'
)
parser.add_argument(
    "--num-batches-per-iter", type=int, default=1,
...
parser.add_argument('--data_dir', type=str, default="/users/Master/alpa/workloads/lucid/lstm/data", help='Data directory')
parser.add_argument('--emsize', type=int, default=200, help='size of word embeddings')
parser.add_argument('--nhid', type=int, default=200, help='number of hidden units per layer')
parser.add_argument('--nlayers', type=int, default=2, help='number of layers')
parser.add_argument(
    '--nhead', type=int, default=2,
...
def benchmark_lstm(model_name, batch_size, mixed_precision, gpu_id, bench_list):
    t_start = time.time()
    if len(gpu_id) == 1:
    ...
    if model_name == 'Transformer':
        ...
        model = models.RNNModel(
            model_name, ntokens, args.emsize, args.nhid, args.nlayers, args.dropout, args.tied
        ).to(device)
...
if __name__ == "__main__":
    ...
    gpu_id = [0, 1, 2, 3]
    bench_list = []
    benchmark_lstm(model_name, batch_size, mixed_precision, gpu_id, bench_list)
    print(bench_list)
