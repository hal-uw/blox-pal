...
# Training
def benchmark_dcgan(model_name, batch_size, mixed_precision, gpu_id, bench_list):
    t_start = time.time()
    if len(gpu_id) == 1:
...
if __name__ == "__main__":
    ...
    model_name = 'DCGAN'
    batch_size = 64  # for dp the global batch size is 64, for ddp the global batch size is 64 * ngpus
    mixed_precision = 0
    gpu_id = [0, 1, 2, 3]
    bench_list = []
    benchmark_dcgan(model_name, batch_size, mixed_precision, gpu_id, bench_list)
    print(bench_list)
